...
```mermaid
flowchart TB
    subgraph Risks["❌ Risks of AI-Generated Traditional Code"]
        direction LR
        ...
        subgraph Governance["Governance Risks"]
            G1["No audit trail"]
            G2["Compliance gaps"]
            G3["Liability unclear"]
            G4["Review burden"]
        end
    end

    Risks --> Result["⚠️ Enterprises hesitant to deploy AI-generated code"]

    style Risks fill:#ffebee
    style Security fill:#ffcdd2
    style Quality fill:#ffcdd2
    style Governance fill:#ffcdd2
...
```mermaid
flowchart TB
    subgraph Mendix["✅ Mendix: Safe AI-Generated Applications"]
        direction LR
        subgraph Security["Security Built-in"]
            S1["Platform handles<br/>SQL, XSS, auth"]
            S2["Role-based<br/>access control"]
            S3["Secure by default"]
        end
        subgraph Quality["Quality Assured"]
            Q1["Model validation<br/>catches errors early"]
            Q2["Runtime safeguards"]
            Q3["Type safety"]
        end
        subgraph Governance["Governance Ready"]
            G1["Full audit trail<br/>in model"]
            G2["Visual review<br/>in Studio Pro"]
            G3["Compliance<br/>certifications"]
        end
    end

    Mendix --> Result["✅ Enterprises confidently deploy AI-generated Mendix apps"]

    style Mendix fill:#e8f5e9
...
flowchart TB
    ...
    subgraph Tools["Tool Layer"]
        mxcli["mxcli<br/>• MDL parsing<br/>• DSL + Diff<br/>• Model validation"]
        FileTools["Standard Tools<br/>• File read/write<br/>• Git operations<br/>• Code formatting"]
    end

    subgraph Project["Mendix Project"]
        Model["model/*.json"]
        Java["javasource/"]
        JS["javascriptsource/"]
        Theme["themesource/"]
...
```mermaid
flowchart TB
    subgraph IDE["Agentic IDE Layer"]
        direction LR
        Claude["Claude Code"]
        Cursor["Cursor"]
        Windsurf["Windsurf"]
    ...
    subgraph Tooling["MDL Tooling Layer"]
        direction TB
        subgraph LSPLayer["Language Server"]
            direction LR
            LS["MDL Language Server<br/>• Diagnostics<br/>• Completion<br/>• Symbols<br/>• Hover"]
            VSCode["VS Code Extension<br/>• Syntax Highlighting<br/>• Outline View<br/>• Error Display"]
            LS <-->|LSP| VSCode
        end

        subgraph CLI["REPL / CLI Layer"]
            direction LR
            mxcli["mxcli<br/>• exec • check<br/>• lint • diff<br/>• search • init"]
            REPL["REPL Daemon<br/>• Execute<br/>• Session<br/>• Fast exec"]
            CATALOG["CATALOG<br/>• Query • Search<br/>• REFS • Callers<br/>• Impact • Context"]
        end

        LSPLayer --> CLI
    end

    subgraph Platform["Mendix Platform Layer"]
        direction LR
        SDK["Model SDK<br/>• Read/Write<br/>• Transform"]
        Validation["Validation<br/>• Syntax<br/>• Semantic<br/>• Best practices"]
        Runtime["Runtime<br/>• Security<br/>• Scaling<br/>• Monitor"]
        StudioPro["Studio Pro<br/>• Visual Review<br/>• Edit"]
    end
...
```mermaid
flowchart LR
    subgraph S1["1. Syntax"]
        P1["ANTLR4 Parser<br/>• Lexer errors<br/>• Parse errors"]
    end

    subgraph S2["2. Semantic"]
        P2["mxcli check<br/>• Type checking<br/>• Variable scope<br/>• References"]
    end

    subgraph S3["3. Linting"]
        P3["mxcli lint<br/>• Best practices<br/>• SARIF output<br/>• CI/CD ready"]
    end

    subgraph S4["4. Platform"]
        P4["Mendix Validate<br/>• Studio Pro<br/>• mxbuild<br/>• Visual review"]
    end

    P1 -->|"mxcli check<br/>(immediate)"| P2
    P2 -->|"--references"| P3
    P3 -->|"CI/CD"| P4
```
...
flowchart TB
    subgraph Request["1. User Request"]
        User["'Add validation to Customer<br/>for email and credit limit'"]
    end

    subgraph Selection["2. Skill Selection"]
        Skills["validation.md ✓ | entity.md | page.md | microflow.md"]
        Match["Selected: validation.md<br/>(matches 'validation' in request)"]
    end

    subgraph Content["3. Skill Content"]
        Pattern["Pattern:<br/>• Name: VAL_{EntityName}<br/>• Returns: Boolean<br/>• Use VALIDATION FEEDBACK"]
    end

    subgraph Output["4. AI Generation"]
        Generated["CREATE MICROFLOW VAL_Customer<br/>with email + credit limit validation"]
    end

    Request --> Selection --> Content --> Output
...
graph TB
    subgraph Pages
        Page["Page<br/>Order_Overview"]
        Widget["Widget<br/>DataGrid"]
    end

    subgraph Entities
        Order["Entity<br/>Order"]
        Customer["Entity<br/>Customer"]
        Amount["Attr: Amount"]
        Email["Attr: Email"]
    end

    subgraph Microflows
        ACT["Microflow<br/>ACT_Open"]
        SUB["Microflow<br/>SUB_Validate"]
    end
...
flowchart TB
    subgraph Interfaces["Query Interfaces"]
        direction LR
        SQL["SQL Catalog<br/>SELECT * FROM<br/>CATALOG.ENTITIES"]
        SPARQL["SPARQL<br/>PREFIX mx:<br/>SELECT ?s"]
        Cypher["Cypher<br/>MATCH (n)<br/>-[:REL]->"]
        GraphQL["GraphQL<br/>query {<br/>entity { } }"]
    end

    subgraph Graph["Unified Graph Model"]
        InMemory["In-Memory Graph<br/>• Nodes: Entity, Attribute, Microflow, Page<br/>• Edges: association, uses, contains, calls<br/>• Properties: name, type, documentation"]
        SDK["Mendix Model SDK<br/>(source of truth)"]
        InMemory --> SDK
    end
...
```mermaid
flowchart TB
    Task["AI Agent Task:<br/>'Add phone number to all<br/>customer-related entities'"]

    subgraph Step1["Step 1: Discover Related Entities"]
        Q1["MATCH (c:Entity {name: 'Customer'})<br/><-[:ASSOCIATION*1..2]-(related)"]
        R1["Result: Customer, CustomerAddress,<br/>CustomerContact, CustomerNote, Order"]
    end

    subgraph Step2["Step 2: Check Existing"]
        Q2["MATCH (e:Entity)-[:HAS_ATTRIBUTE]-><br/>(a:Attribute {name: 'Phone'})"]
        R2["Result: Only CRM.Customer has Phone"]
    end

    subgraph Step3["Step 3: Generate MDL"]
        MDL["ALTER ENTITY CRM.CustomerAddress ADD Phone<br/>ALTER ENTITY CRM.CustomerContact ADD Phone<br/>ALTER ENTITY CRM.CustomerNote ADD Phone<br/>ALTER ENTITY Sales.Order ADD Phone"]
    end

    subgraph Step4["Step 4: Validate & Execute"]
        Validate["mxcli check → mxcli exec"]
    end
...
```mermaid
flowchart TB
    subgraph LS["MDL Language Server"]
        Parser["ANTLR4 Parser<br/>• Tokenization<br/>• AST generation<br/>• Error recovery<br/>• Cross-language"]
        Providers["LSP Providers<br/>• Diagnostics<br/>• Completion<br/>• Symbols<br/>• Hover<br/>• Code Actions"]
        Parser --> Providers
    end

    subgraph Human["👤 Human Developer (VS Code)"]
        H1["Diagnostics → Red squiggles"]
        H2["Completion → Autocomplete"]
        H3["Symbols → Outline panel"]
        H4["Hover → Documentation"]
        H5["Code Actions → Quick fixes"]
    end

    subgraph AI["🤖 AI Coding Agent (Claude Code)"]
        A1["Diagnostics → Self-correction"]
        A2["Completion → Context-aware gen"]
        A3["Symbols → Code understanding"]
        A4["Hover → API lookup"]
        A5["Code Actions → Auto-repair"]
    end
...
```mermaid
flowchart LR
    subgraph Step1["1. User Request"]
        User["'Create a CRM with<br/>Customer and Order<br/>tracking'"]
    end

    subgraph Step2["2. AI Generation"]
        AI["Claude Code<br/>generates MDL<br/>• Entities<br/>• Associations<br/>• Microflows<br/>• Pages"]
    end

    subgraph Step3["3. Human Review"]
        Review["VS Code + MDL<br/>• Syntax colored<br/>• Errors shown<br/>• Outline view"]
    end

    subgraph Step4["4. Execute MDL"]
        REPL["REPL Daemon<br/>• Apply to model<br/>• Validation<br/>• Commit"]
    end

    subgraph Step5["5. Visual Review"]
        Studio["Studio Pro<br/>• Domain model<br/>• Review pages<br/>• Test flows"]
    end

    subgraph Step6["6. Deployment"]
        Cloud["Mendix Cloud<br/>• One-click deploy<br/>• Staging env<br/>• Production"]
    end

    User --> AI --> Review --> REPL --> Studio --> Cloud
...
```mermaid
flowchart TB
    subgraph L1["Layer 1: MDL Syntax"]
        S1["mxcli check<br/>• Lexer errors<br/>• Parser errors<br/>• Immediate feedback"]
    end

    subgraph L2["Layer 2: Semantic Validation"]
        S2["mxcli check --references<br/>• Type checking<br/>• Reference validation<br/>• Variable scope<br/>• Return requirements"]
    end

    subgraph L3["Layer 3: Linting & Best Practices"]
        S3["mxcli lint<br/>• Naming conventions<br/>• Empty microflow detection<br/>• Domain model limits<br/>• SARIF output"]
    end

    subgraph L4["Layer 4: Platform Validation"]
        S4["Studio Pro / mx check<br/>• Cross-module refs<br/>• Security config<br/>• Widget compatibility"]
    end

    subgraph L5["Layer 5: Runtime Checks"]
        S5["Mendix Runtime<br/>• Input validation<br/>• Access control<br/>• Transaction integrity"]
    end
...
flowchart TB
    subgraph Analyze["1. ANALYZE"]
        Sources["OutSystems | Java/.NET | Database | UI Screens"]
        AI1["AI Agent<br/>• Parse source artifacts<br/>• Extract domain model<br/>• Identify business rules<br/>• Map UI patterns"]
        Sources --> AI1
    end

    subgraph Generate["2. GENERATE MDL"]
        Domain["Domain Model<br/>CREATE ENTITY Sales.Customer"]
        Logic["Business Logic<br/>CREATE MICROFLOW CalculateDiscount"]
        UI["User Interface<br/>CREATE PAGE Customer_Overview"]
    end

    subgraph Validate["3. VALIDATE & REFINE"]
        Val["• mxcli check (syntax)<br/>• mxcli exec (apply)<br/>• Studio Pro (visual review)<br/>• Human adjusts<br/>• AI iterates"]
    end

    subgraph Deploy["4. DEPLOY & VALIDATE"]
        D1["Deploy to Test"]
        D2["Test with Legacy"]
        D3["Compare Output"]
        D4["Go Live"]
...
flowchart TB
    subgraph Input["User Request"]
        U["User: Migrate this OutSystems module to Mendix<br/><i>Provides: OML export file or documentation</i>"]
    end

    subgraph Claude["Claude Code Processing"]
        C1["1. Parse OutSystems OML/metadata"]
        C2["2. Extract entity definitions → CREATE ENTITY"]
        C3["3. Map references → CREATE ASSOCIATION"]
        C4["4. Convert Server Actions → CREATE MICROFLOW"]
        C5["5. Transform Screens → CREATE PAGE"]
        C6["6. Validate via Language Server"]
        C7["7. Execute via REPL to create Mendix model"]
        C8["8. Show in Studio Pro for human review"]
    end
...
flowchart TB
    subgraph Input["User Request"]
        U["User: Migrate this Spring Boot application to Mendix<br/><i>Provides: GitHub repo URL or source files</i>"]
    end

    subgraph Claude["Claude Code Processing"]
        C1["1. Scan codebase structure"]
        C2["2. Parse JPA entities<br/>@Entity Customer → CREATE ENTITY"]
        C3["3. Parse relationships<br/>@ManyToOne → CREATE ASSOCIATION"]
        C4["4. Analyze service methods<br/>calculateTotal() → CREATE MICROFLOW"]
        C5["5. Map REST endpoints<br/>GET /customers → CREATE PAGE"]
        C6["6. Generate complete MDL script"]
        C7["7. Validate and iterate"]
    end
...
block-beta
    ...
    block:editor["VS Code Editor"]:2
        columns 2
        source["Source (Read-Only)<br/>─────────────────<br/>@Entity<br/>public class Customer {<br/>  @Column(length = 200)<br/>  private String name;<br/>  private String email;<br/>  private BigDecimal creditLimit;<br/>}"]
        mdl["Generated MDL<br/>─────────────────<br/>CREATE PERSISTENT ENTITY<br/>  CRM.Customer (<br/>  Name: String(200) NOT NULL,<br/>  Email: String(200),<br/>  CreditLimit: Decimal<br/>);<br/><br/>✓ Syntax highlighting<br/>✓ Error checking<br/>✓ Outline view"]
    end

    block:preview["MDL Preview Panel"]:2
        tree["Entity: CRM.Customer<br/>├─ Name: String(200) NOT NULL<br/>├─ Email: String(200)<br/>└─ CreditLimit: Decimal"]
        actions["[Open in Studio Pro]<br/>[Execute in REPL]<br/>[Validate]"]
    end
```
...
flowchart TB
    subgraph P1["Phase 1: Discovery"]
        P1A["AI analyzes legacy codebase"]
        P1B["Generates inventory: entities, services, screens"]
        P1C["Identifies patterns and complexity"]
        P1D["Produces migration assessment report"]
    end

    subgraph P2["Phase 2: Domain Model Migration"]
        P2A["Generate entities from DB schema / entity classes"]
        P2B["Generate associations from foreign keys / relationships"]
        P2C["Generate enumerations from lookup tables / enums"]
        P2D["Validate in Studio Pro, adjust as needed"]
    end

    subgraph P3["Phase 3: Business Logic Migration"]
        P3A["Generate microflows from service methods"]
        P3B["Convert stored procedures to microflows"]
        P3C["Map validation rules"]
        P3D["Human review of complex logic"]
    end

    subgraph P4["Phase 4: UI Migration"]
        P4A["Generate pages from screens/forms"]
        P4B["Map UI patterns to Mendix widgets"]
        P4C["Apply Atlas UI styling"]
        P4D["User acceptance testing"]
    end

    subgraph P5["Phase 5: Integration & Cutover"]
        P5A["Connect to existing systems (if needed)"]
        P5B["Data migration"]
        P5C["Parallel running"]
...
flowchart TB
    subgraph MOAT["Competitive Moat"]
        Open["Open Platform<br/>No vendor lock-in"]
        BYOA["Bring Your Own Agent<br/>Any AI, any workflow"]
        Collab["Human + AI Collaboration<br/>Review & refine together"]
        Speed["Hours, Not Weeks<br/>Complex tasks simplified"]
        Coexist["Co-existence Strategy<br/>Best of both worlds"]
    end

    Open --> Trust["Enterprise Trust"]
    BYOA --> Future["Future-Proof"]
    Collab --> Quality["Quality Assurance"]
    Speed --> ROI["Rapid ROI"]
    Coexist --> Adoption["Easy Adoption"]

    Trust --> Win["Market Leadership"]
    Future --> Win
    Quality --> Win
    ROI --> Win
    Adoption --> Win

    style MOAT fill:#e3f2fd
...
```mermaid
flowchart LR
    subgraph AI["AI Agent Strengths"]
        A1["Generate boilerplate"]
        A2["Apply patterns at scale"]
        A3["Bulk modifications"]
        A4["Migration automation"]
    end

    subgraph Human["Human Strengths"]
        H1["Business judgment"]
        H2["Edge case handling"]
        H3["UX refinement"]
        H4["Final approval"]
    end

    subgraph Collab["Collaboration Points"]
        C1["MDL Review<br/>(scannable diff)"]
        C2["Studio Pro<br/>(visual verification)"]
        C3["mxcli lint<br/>(automated checks)"]
    end
    ...
    Human --> C2
    C2 --> C3
    C3 --> Deploy["Production"]

    style AI fill:#fff3e0
    style Human fill:#e8f5e9
    style Collab fill:#e3f2fd
```
